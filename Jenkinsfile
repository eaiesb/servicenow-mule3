...
steps {
          ...
          slackSend (color: "add8e6", message: 'servicenow-mule3 Deployment Started')
          buildsrc() 
          slackSend (color: "0000ff", message: 'servicenow-mule3 Build Sucessfully')
...
stage('upload to atifactory') {
  ...
  "files": [
    {
      "pattern": "**/*.jar",
      "target": "generic-local/mule3-demo-jars_$BUILD_NUMBER/servicenow-mule3.jar"
    }
 ]
}"""                 
              def buildInfo1 = server.upload spec: uploadSpec
                      slackSend (color: "#FFA500",message: 'servicenow-mule3 Artifacts Uploaded Sucessfully')
            }
    }
}  	
}
   post {
      failure {
            emailext attachLog: true, body: 'Deployment has failed', subject: 'servicenow-mule3 Deployment Status', to: 'sudheekarreddy.donapati@eaiesb.com'
           slackSend (color: "#FF0000",message: 'servicenow-mule3 Deployment Failed')
        }
      success {
...
''', subject: 'Jenkins ${BUILD_STATUS} [#${BUILD_NUMBER}] - ${PROJECT_NAME} ${ENV, var="GIT_URL"}', to: 'sudheekarreddy.donapati@eaiesb.com'    
          slackSend (color: "#32CD32", message: 'servicenow-mule3 Deployment is Sucessful')
        }
  }
}
// steps
